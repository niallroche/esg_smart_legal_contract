...
# Accord Protocol Template: sample
...
```
cicero parse --template ./sample/ --dsl ./sample/sample.txt
...
data.json:
{
   "$class": "com.acme.test.MyRequest",
   "input": "World"
}
```

```
cicero execute --template ./sample/ --dsl ./sample/sample.txt --data ./sample/data.json 
```
...
```
{
   ...
   "request":{
      "$class":"com.acme.test.Request",
      "input":"World"
   },
   "response":{
      "$class":"com.acme.test.Response",
